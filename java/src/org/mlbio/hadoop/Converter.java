...
/*
 * Convert dataset from libSVM format to Hadoop Sequence File
 * This is easier than parsing the input file in every iteration 
 * also the file size is reduced
 * 
 * Key-value pair
 * Map - converts the input line in libSVM format to (Int, Example) pair.
 *      key = line# (probably, it doesn't matter)
 *      value = One line from libSVM file
 *      
 * Reducer - single identity reducer, all map output is sent to one reducer by using a 
 * single key.
 * 
 */
